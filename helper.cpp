...
double get_G_hat_cpp(double t, double x, double u, List valid_info,
  ...
  std::vector<int> idx;
  for(int i=0;i<n;i++){
    // floating issue use tol
    if (delta_tx[i]==1 && Q_t[i] > x + tol && T_tx[i]>=u)
      idx.push_back(i);
  }
  ...
  for(int i : idx) {
    ...
    G_num += 1.0 / H;
    //Rcpp::Rcout << "G_num: " << G_num << std::endl;
  }
  G_num /= n;
...
double get_var_inner_cpp(double t, double tprime, double x, double xprime, 
  ...
  //Rcpp::Rcout << "G_hat: " << G_hat << std::endl;
  //Rcpp::Rcout << "G_hat_prime: " << G_hat_prime << std::endl;
  //Rcpp::Rcout << "H_hat: " << H_hat << std::endl;
  //Rcpp::Rcout << "H_hat_prime: " << H_hat_prime << std::endl;
